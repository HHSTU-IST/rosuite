  ...
  public:
        ...
        std::optional<core::ControlInput> control = std::nullopt) const override
    {
      ...
      for (core::Index i = 0; i < ensemble_size_; ++i)
      {
        ...
        const auto noise =
            rng.sample_multivariate_normal(Vector::Zero(estimate.dimension()),
                                           process_noise.value());
        if (!noise.ok())
        {
          return noise.status();
        ...
        const models::ModelContext &context = {}) const override
    {
      ...
      for (core::Index i = 0; i < ensemble_size_; ++i)
      {
        ...
        if (!initialized)
        {
          measurement_ensemble = Matrix::Zero(
              predicted.value().measurement.dimension(), ensemble_size_);
          initialized = true;
        }
      ...
      Covariance innovation_covariance = core::symmetrize(
      ...
      const Matrix gain = Matrix(cross_covariance * ldlt.solve(
      ...
      for (core::Index i = 0; i < ensemble_size_; ++i)
      {
        const auto perturbation = rng.sample_multivariate_normal(
            Vector::Zero(measurement.dimension()), measurement_noise.value());
        if (!perturbation.ok())
        {
          return perturbation.status();
        }

        state_ensemble.col(i) += gain * (measurement.value + perturbation.value() - measurement_ensemble.col(i));
      }
